...
let options = {"images":true,"videos":true,"audios":true,"iframes":true,"polyfill":true,"directiveOnly":false,"loadedClass":"isLoaded","appendClass":"lazyLoad","observerConfig":{},"globalName":"nuxt"};
let observer = null;
    ...
    if(isWindowLoaded){
      entries.forEach(entry => {
        if(entry.isIntersecting){
          if(entry.target.getAttribute('lazy-background')){
            setBackground(entry.target);
          } else {
            setAttributes(entry.target);
            setLoadedClass(entry.target)
