          ...
          <div class="sub-category-navigation">
            ...
            <ul class="sub-category-navigation__nav">
              <template v-if="category && category.children">
                ...
                >
                  <nuxt-link
                    :to="localePath({ path: `/category-list/${slug}` })"
          ...
          <div class="sub-category-cards">
            <template v-if="category && category.children">
              <nuxt-link
                ...
                :key="`card-${slug}`"
                :to="localePath({ name: 'category-list-slug', params: { slug: slug } })"
                class="category-card"
              >
...
export default {
  ...
  mounted () {
    console.log('PAGE :: _sub-category-lug')
    this.fetchCategory()
  },
  methods: {
    async fetchCategory () {
      this.category = (await this.$axios.$get(config.apiUrl + `sub-catalog/${this.$nuxt._route.params.slug}?lang=${this.$i18n.locale}`)).data
      this.breadCrumbsData = [
        {
          title: this.category.parent_translation,
          url: this.localePath({ name: 'catalog-slug', params: { slug: this.category.parent_slug } })
        },
        {
          title: this.category.short !== '' ? this.category.short : this.category.translation
        }
      ]
    }
  }
